...
def _dspark_table_block_size(
    ...
    """
    physical_blocks = (int(max_model_len) + int(physical_block_size) - 1) // int(physical_block_size)
    table_width = int(block_table.shape[1])
    if physical_blocks <= 0 or table_width % physical_blocks != 0:
        raise ValueError(
            "DSpark block-table width is inconsistent with the configured context: "
            f"width={table_width}, max_model_len={max_model_len}, "
            f"physical_block_size={physical_block_size}."
        )
    blocks_per_phys_block = table_width // physical_blocks
    if blocks_per_phys_block <= 0 or physical_block_size % blocks_per_phys_block != 0:
        raise ValueError(
            "DSpark hybrid block factor must divide the physical block size: "
            f"factor={blocks_per_phys_block}, physical_block_size={physical_block_size}."
        )
...
class AscendDSparkProposer(AscendDflashProposer):
    ...
    def __init__(
        ...
        # base defaults this value to token 0, which produces essentially
        # random draft logits and collapses acceptance.
        ...
        for attn_group in draft_attn_groups:
            ...
            if HAS_TRITON:
                ...
                if os.getenv("VLLM_ASCEND_DSV4_310P_VALIDATE_SLOTS", "0") == "1":
                    query_slots_cpu = (
                        self._per_group_query_slot_mapping_buffers[gid][:num_query_total].to("cpu").tolist()
                    )
                    context_slots_cpu = (
                        self._per_group_slot_mappings[gid][: self._dflash_num_context].to("cpu").tolist()
                    )
                    logger.warning(
                        "DSpark 310P slot geometry gid=%d physical_block_size=%d table_block_size=%d "
                        "seq_lens=%s query_start=%s block_table=%s "
                        "context_slots=%s query_slots=%s",
                        gid,
                        kv_block_size,
                        table_block_size,
                        cad.seq_lens[:batch_size].to("cpu").tolist(),
                        cad.query_start_loc[: batch_size + 1].to("cpu").tolist(),
                        gid_block_table[:batch_size].to("cpu").tolist(),
                        context_slots_cpu,
                        query_slots_cpu,
                    )
