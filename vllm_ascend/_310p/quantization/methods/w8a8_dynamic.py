...
class AscendW8A8DynamicLinearMethod310(AscendW8A8Linear310pScheme):
    ...
    def apply(
        ...
        # ``npu_quant_matmul`` requires a 2-D activation and a 1-D per-token
        # scale. MTP carries candidate dimensions in front of the hidden size,
        # so flatten those dimensions for the quantized GEMM and restore them
        # afterwards.
        input_shape = x.shape
